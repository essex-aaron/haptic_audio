//tried to play with sample rates and buffer
...
//  if (ind > 450 && ind < 500) pitch = 1.0f;
//  else if (ind == 500) { pitch = 1.0f; ind = 0; }
//  else pitch = 0.0f;
//  ind ++;
//  if (pitch > MAX) pitch = MAX;//pressure2 * 1;
//  else if(pitch < MIN) pitch = MIN;
//  pitch2 = pressure * 5;
  ROS_INFO("Output Val: [%d]", ind);
  ROS_INFO("Output2 Val: [%f]", pitch);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = n.subscribe("/rdda_master_input", 1, packet_msgCallback);
...
//As it is now should subscribe to packet_msg topic and display readings //need to import soundio library into rdda cmakelist
//need to figure out how to edit sio_sine such that it looks for inputs instead of static sine wave